		...
		for (k, mut state) in beam {
			...
			if let Some(p) = p {
				...
				if h + 1 == data.input.hole.len() && !has_edge[state.ps[0]][p] {
					k.0 += 1;
					k.1 += (data.dist[state.ps[0]][p] * 1e8) as i64;
				} else if h + 1 == data.input.hole.len() {
					let before = (data.input.figure.vertices[state.ps[0]] - data.input.figure.vertices[p]).abs2() as f64;
					let after = (data.input.hole[0] - data.input.hole[h]).abs2() as f64;
					k.1 += ((after / before - 1.0).abs() * 1e8) as i64;
				}
				if !has_edge[state.ps[h - 1]][p] {
					k.0 += 1;
					k.1 += (data.dist[state.ps[h - 1]][p] * 1e8) as i64;
				} else {
					let before = (data.input.figure.vertices[state.ps[h - 1]] - data.input.figure.vertices[p]).abs2() as f64;
					let after = (data.input.hole[h - 1] - data.input.hole[h]).abs2() as f64;
					k.1 += ((after / before - 1.0).abs() * 1e8) as i64;
				}
				state.ps.push(p);
				next.push((k, state));
